 ...
 * -----
 * Last Modified: 2022-11-14 10:57:46 am
...
import tp.common.tools.{Sample, Counter}
...
class Rx (p: UartParams) extends Module {
  ...
  switch (r_fsm) {
    ...
    is(s2DATA) {
      when (m_ccnt.io.o_flag) {
        r_data(m_dcnt.io.o_val) := m_sample.io.o_data
